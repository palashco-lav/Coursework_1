...
def test_spending_by_category(transactions_df: pd.DataFrame, category: str, expected_sum: pd.DataFrame) -> None:
...
def test_missing_columns(transactions_df: pd.DataFrame) -> None:
...
def test_invalid_date_format(transactions_df: pd.DataFrame) -> None:
    with pytest.raises(ValueError):
