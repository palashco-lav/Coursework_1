...
def get_last_card_total_cashback(read_data : pd.DataFrame,
                                 start_date_ts : pd.DatetimeIndex,
                                 operation_date_ts : pd.DatetimeIndex) -> dict:
    """
    Функция получения данных о последних 4-х цифрах номера карты, общая сумма расходов, кешбэк (1 рубль на каждые 100 рублей)
    ...
    logger_utils.debug(f'Обрабатываем данные с {start_date_ts.strftime("%Y-%m-%d %H:%M:%S")} по {operation_date_ts.strftime("%Y-%m-%d %H:%M:%S")}')
    ...
    for card in user_card:
        if isinstance(card, str) and len(card) >= 4 and card[-4:].isdigit():
            ...
            logger_utils.debug(f'Отфильтрованные данные: '
...
def get_top_transactions(df: pd.DataFrame) -> dict:
    ...
    logger_utils.debug(f'Формируем словарь топ-5 транзакций:"')
    for index, row in min_values.iterrows():
        ...
        logger_utils.debug(f'   Дата операции: {top_transactions_dict['date']}'
...
def get_currency_rates(currencies):
    ...
    logger_utils.debug(f'Запрашиваю данные по курсам валют с сервера ЦБ: https://www.cbr-xml-daily.ru/daily_json.js')
    ...
    logger_utils.debug(f'Получаем список нужных валют')
...
def get_stock_prices(user_stocks: list) -> dict:
    ...
    # Делаем запросы для каждой акции с задержкой в 15 секунд
    # Это необходимо, так как Alpha Vantage имеет ограничение на количество запросов
